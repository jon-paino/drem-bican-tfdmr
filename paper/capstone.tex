...
\title{Extending the Dynamic Regulatory Events Miner (DREM) to
       Methylation Dynamics: A TF--DMR Prior Framework for Brain
       Cell Lineages}
...
\begin{figure*}[t]
  ...
  \caption{DREM model fit for the DRD1-BACH2 trajectory using the regenerated TF--DMR prior.
    ...
    Split nodes are annotated with the transcription factors that
    the logistic regression at the split identifies as significant
    discriminators between genes assigned to the outgoing paths.}
...
\begin{figure*}[t]
  ...
  \caption{DREM model fits for the DRD1-EPHA4 , eCGE-LAMP5, and MGE-PVALB trajectories using the
    ...
    developmental stages advancing left-to-right and split nodes
    annotated with the transcription factors the logistic regression
    identifies as significant discriminators.}
...
% TODO Jon: confirm specific TFs above match what appears on the
% figure splits; the list above reflects the BICAN published TF
% associations for each lineage but should be cross-checked against
% the actual DREM annotations in the corresponding figure.
